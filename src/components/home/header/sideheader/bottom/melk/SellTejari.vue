...
export default {
  ...
  setup() {
    ...
    const sellTejariMarks = computed(() => info.subItem.price_tejari);
    const sellTejariPrice_items = ref(["قیمت کل", "حداقل", "حداکثر"]);

    const sellTejariSpaceMarks = computed(() => info.subItem.space_sell_tejari);
